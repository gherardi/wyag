...
class GitRepository(object):
    ...
    def __init__(self, path, force=False):
        ...
        # if we are not in force mode (force), .git must exist
...
class GitObject(object):
    """
    Base class for Git objects (blobs, trees, commits, tags).
    """
    ...
    def serialize(self, repo):
        """
        Serializes the object in bytes (implemented by subclasses).
        """
        raise Exception("Unimplemented!")

    def deserialize(self, data):
        """
        Deserializes data in memory (implemented by subclasses).
        """
        raise Exception("Unimplemented!")
...
class GitCommit(GitObject):
    ...
    def deserialize(self, data):
        # parsing the key-value fields of the commit and tags
        self.kvlm = kvlm_parse(data)

    def serialize(self):
        # serialization of the key-value dictionary
        return kvlm_serialize(self.kvlm)

    def init(self):
        # initialize empty dictionary for new commit
        self.kvlm = dict()

class GitTreeLeaf(object):
    """
    Represents an entry in a Git tree: (mode, path, sha)
    """
...
# kvlm_parse stands for Key-Value List with Message
# I don't use commit_parse because tags have the very same format, so we’ll use it for both objects types
...
# 2. the “real” parser which just calls the previous one in a loop, until input data is exhausted
